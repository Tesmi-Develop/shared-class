...
const remotes = createRemotes({
	...
	CreateSharedInstance: remote<
		Client,
		[SharedClassId, string, unknown[], Map<string, unknown>, ObjectKey | undefined]
	>(t.string, t.string, t.array(t.any), t.map(t.string, t.any), t.optional(t.union(t.string, t.number))),
	SharedClassesCommunicationServer: remote<Client, [SharedClassId, PackageTypeServer, object]>(
		t.string,
		t.number,
		t.table,
	),
	SharedClassesCommunicationClient: remote<Server, [SharedClassId, PackageTypeClient, object]>(
		t.string,
		t.number,
		t.table,
	),
	SharedClassesUnreliableCommunicationServer: remote<Client, [SharedClassId, PackageTypeServer, object]>(
		t.string,
		t.number,
		t.table,
	).unreliable(),
	SharedClassesUnreliableCommunicationClient: remote<Server, [SharedClassId, PackageTypeClient, object]>(
		t.string,
		t.number,
		t.table,
	).unreliable(),
...
const SendPackageServer = <D extends object>(
	object: object,
	methodType: PackageTypeServer,
	data: D,
...
const SendPackageClient = <D extends object>(
	object: object,
	methodType: PackageTypeClient,
	data: D,
	isUnreliable: boolean,
) => {
...
const startHydration = (context: object, rate: number) => {
	return task.spawn(() => {
		...
		while (true) {
			...
			SendPackageServer(
				context,
				PackageTypeServer.hydration,
				{
					properties: generateReplicatedProperties(context),
				},
				false,
			);
		}
...
const assertInvalidArgument = (argType: string) => {
	throw `\n[SharedClass] Not supported by arguments of the type ${argType}`;
};

const notSupportedArgumentTypes: (keyof CheckableTypes)[] = ["function", "thread"];

const validateSharedClassArgument = (argument: defined) => {
	notSupportedArgumentTypes.includes(typeOf(argument)) && assertInvalidArgument(typeOf(argument));

	if (typeIs(argument, "table")) {
		for (const [key, value] of pairs(argument)) {
			validateSharedClassArgument(value);
		}
	}
};

const validateSharedClassArguments = (args: defined[]) => {
	args.forEach((value) => {
		validateSharedClassArgument(value);
	});
};

const HaveDestroyMethod = (object: object): object is { Destroy: Callback } =>
	"Destroy" in object && typeIs(object.Destroy, "function");
...
export const SharedClass = <T extends object, K extends keyof ExtractMethods<T>, D extends ExtractMethods<T>>({
	...
	return (sharedClass: Constructor<T>) => {
		...
		objectWithconstructor.constructor = function (this, ...args: defined[]) {
			validateSharedClassArguments(args);
			...
			if (isServer) {
				...
				remotes.CreateSharedInstance.fireAll(
					instanceData.Id,
					className,
					args,
					generateReplicatedProperties(this),
					ClientMethodInitName,
				);

				HydrationRate && maid.GiveTask(startHydration(this, HydrationRate));

				maid.GiveTask(
					OnRequestSharedClasses.Connect((player) => {
						remotes.CreateSharedInstance.fire(
							player,
							instanceData.Id,
							className,
							args,
		...
		if (DestroyMethodName || HaveDestroyMethod(objectWithconstructor)) {
			...
			objectWithDestroy[(DestroyMethodName || "Destroy") as never] = function (
				context: typeof objectWithconstructor,
				...args: defined[]
			) {
				if (isServer) {
					SendPackageServer(
